...
class game():
	...
	def grid(self):
		self._grid=self.canvas.create_rectangle(0+self.margin*2-1,0+self.margin*2-1,self.canvasWidth-self.margin*2+1,self.canvasHeight-self.margin*2+1,tags='grid')
	...
	def add_fruits(self):
		...
		y1=random.randrange(0+self.margin*3+1, self.canvasHeight-self.margin*3-1,self.margin)
	...
	def snake_move(self):
		...
		if self.move!=0:
			...
			#narazenie
			tagged_objects = self.canvas.find_withtag('grid')
			overlapping_objects = self.canvas.find_overlapping(*self.canvas.coords('snake'+str(self.n)))
			...
			for item in overlapping_objects:
				if item in tagged_objects:
					...
					self.control=0
			#zjedenie seba
			if self.n-1 - self.i>5:
				for i in range(self.i-5,self.n-5):
					tagged_objects = self.canvas.find_withtag('snake'+str(i))
					overlapping_objects = self.canvas.find_overlapping(*self.canvas.coords('snake'+str(self.n)))
